    ...
    <div class="container-fluid">
      <div class="row">
        <v-container class="mt-3">
          <v-card color="white">
            <div class="row">
              <div class="col-12">
                <v-card-title class="text-h5"> <v-icon>mdi-application-cog</v-icon> {{category.name}}</v-card-title>
      ...
      <div class="row">
        <div class="col-sm-7 col-xs-12">
          <h2 class="text-2xl text-right rtl font-bold p-2 text-blue-400"> <v-icon>mdi-application-cog</v-icon> تصفح احدث وافضل التعريفات </h2>
...
export default {
  ...
  methods: {
    infiniteHandler($state) {
      this.$axios.get(process.env.apiUrl+'/drivers'+ '?page=' + this.page)
        .then(response => {
  ...
  asyncData({$axios}) {
    return $axios.$get(process.env.apiUrl+'/drivers')
      .then(response => {
        return{
